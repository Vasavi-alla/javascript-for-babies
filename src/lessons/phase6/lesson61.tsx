...
function BlockedStack({ stepIndex }: { stepIndex: number }) {
  ...
  return (
    <svg viewBox="0 0 440 326" className="w-full">
      ...
      <AnimatePresence mode="wait">
        {view.badge && (
          <motion.g key={view.badge} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            <text x={220} y={219} textAnchor="middle" fontFamily="var(--font-hand)" fontSize={10} fontWeight={700} fill="var(--color-pencil-blue)">
              {view.badge}
            </text>
          </motion.g>
        )}
      ...
      <AnimatePresence mode="wait">
        <motion.text key={view.note} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} x={220} y={254} textAnchor="middle" fontFamily="var(--font-hand)" fontSize={12.5} fontWeight={700} fill={view.frozen ? 'var(--color-marker-coral)' : 'var(--color-marker-teal)'}>
          {view.note}
        </motion.text>
      </AnimatePresence>
